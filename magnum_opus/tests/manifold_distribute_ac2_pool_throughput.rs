//! F5b AC2 - Manifold throughput: IronOre does not pool up without bound.
...
fn iron_ore_pool_remains_bounded() {
    ...
    assert!(ore < 10.0, "Manifold must not accumulate unboundedly ({ore})");
}
